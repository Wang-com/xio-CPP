...
from utils.utils import load_classes, calc_fps
from model.transform import preds_postprocess
...
from yolox.interface.yolo_main import create_model
...
def detect_main(q_thread):
    ...
    # 获取检测模型
    model = create_model(0.3, 0.65, 640)
    MySql.connect_mysql()
    ...
    while True:
        ...
        if curr_time - update_detection_flag_clock_start > update_detection_flag_interval:
            MySql.add_record(mysql_interrupt_table, True)
            update_detection_flag_clock_start = curr_time

        if curr_time - patrol_opc_nodes_clock_start > 20:
            time.sleep(100000)
        ...
        # 返回值只有非None视频流的预测结果
        preds_dict = preds_postprocess(preds_output, preds_info, not_none_streams, classes)
        ...
        handling.handle_judgement(judgements_dict, vis_images_dict)
        ...
        for title, (i, img) in zip(img_title_dict.values(), enumerate(vis_images_dict.values())):
            ...
            if i == 0:
                ...
                q_thread.video_7_change_pixmap.emit(q_image_h, title)
            else:
