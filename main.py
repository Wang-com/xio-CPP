...
from configs.config import station_name_dict, station_name_switch_dict, \
    site_1, site_2, site_3, site_4, site_5, site_6, site_7, dynamic_config_path, interrupt_switch_section
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, detection_flag):
        ...
        self.action_site_7.triggered.connect(self.site_7_switch)
    ...
    def init_interrupt(self):
        ...
        judge7, _ = self.read_interrupt(site_7)
        if not judge1:
        ...
        if not judge7:
            self.action_site_7.setText(station_name_dict[site_7])
        else:
            self.action_site_7.setText(station_name_switch_dict[site_7])
    ...
    @pyqtSlot(QImage, str)
    def set_frame_7(self, image, name):
        self.video_display_7.setPixmap(QPixmap.fromImage(image))
        self.video_title_7.setText(name)
    ...
    @pyqtSlot(bool)
    def site_7_switch(self, trigger):
        judge, config = self.read_interrupt(site_7)
        if judge:
            self.action_site_7.setText(station_name_dict[site_7])
        else:
            self.action_site_7.setText(station_name_switch_dict[site_7])
        self.write_interrupt(site_7, not judge, config)
...
class DetectionThread(QThread):
    ...
    video_7_change_pixmap = pyqtSignal(QImage, str)
